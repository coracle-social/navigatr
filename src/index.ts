...
import {isShareableRelay, Pool, Relays, Relay, Executor} from 'paravel'
...
export class Navigator extends EventEmitter {
  ...
  getExecutor(urls: string[]) {
    return new Executor(new Relays(urls.map(url => {
  ...
  load({urls, filters, onEvent}: {urls: string[], filters: Filter[], onEvent: (e: Event) => void}) {
  ...
  async scrapeAll({minRelays = Infinity, minEvents = Infinity, chunkSize = 10} = {}) {
